...
class Bytecode():
    ...
    def __init__(self, addr, buffer, prev=None, next=None, xrefs=[]):
        self.opcode = ord(chr(buffer[0]))
        self.addr = addr

        if self.opcode >= dis.HAVE_ARGUMENT:
            self.oparg = ord(chr(buffer[1])) | (ord(chr(buffer[2])) << 8)
        else:
            self.oparg = None
...
class BytecodeGraph():
    ...
    def apply_lineno(self):
        ...
        bytecode.
        '''
        byte_increments = [ord(chr(c)) for c in self.code.co_lnotab[0::2]]
        line_increments = [ord(chr(c)) for c in self.code.co_lnotab[1::2]]
    ...
    def get_code(self, start=None):
        ...
        rvalue = PyCode_New(self.code.co_argcount,
    ...
    def parse_bytecode(self):
        ...
        while offset < len(self.code.co_code):
            coff = self.code.co_code[offset:offset+3]
            if len(coff) > 0:
...
def main(argv):
    ...
    for i in range(0, len(ob)):
        with open(str(i)+'.pyc', 'wb') as fc:
            ...
            fc.close()
        with open(str(i)+'.pyc', 'rb') as fa:
            bytecode = base64.b64encode(fa.read())
            print(str(i)+'.pyc;' + str(bytecode, 'utf-8'))
            fc.close()
        os.remove(str(i)+'.pyc')
    return
